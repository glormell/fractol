...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/13 17:40:34 by glormell          #+#    #+#             */
/*   Updated: 2019/08/13 17:40:45 by glormell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_menu_button	*is_main_menu_hook(t_point2 p, t_frc *frc)
{
	...
	if (p.x > m.p.x + m.mb.p.x && p.x < m.p.x + m.mb.p.x + m.mb.s.x &&
		...
		return (&frc->mn.bs);
	else if (p.x > m.p.x + m.tc.p.x && p.x < m.p.x + m.tc.p.x + m.tc.s.x &&
			p.y > m.p.y + m.tc.p.y && p.y < m.p.y + m.tc.p.y + m.tc.s.y)
		return (&frc->mn.tc);
	return (0);
}
